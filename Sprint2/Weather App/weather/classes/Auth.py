...
class Auth:
    ...
    def validate_data(self, user_input: UserData):
        if user_input.username in self.users:
            return False, "Username already exists"

        if len(user_input.username) < 3:
    ...
    def register(self, username, password):
        """Register a new user"""
        userInput = UserData(username=username, password=password)
        is_valid, message = self.validate_data(userInput)
        if not is_valid:
